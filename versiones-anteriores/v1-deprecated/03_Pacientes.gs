...
function _actualizarEstadosFila(row) {
  ...
  if (vital !== 'FALLECIDO') {
    for (var c = 0; c < CONTROL_COLS.length; c++) {
      ...
      var nuevo = calcStatus(_parseDate(_rawFecha), meses, actual, diasAviso, _rawFecha)
...
function recalcularTodo() {
  ...
  var cols = Object.keys(needed).map(Number).sort(function(a,b) { return a - b })
  ...
  for (var ci = 0; ci < CONTROL_COLS.length; ci++) {
    ...
    for (var r = 0; r < rows; r++) {
      ...
      if (ec && ec <= lc) {
        ...
        var nuevo = calcStatus(parsed, meses, actual, diasAviso, raw)
...
function probarDiasAviso() {
  ...
  var cols = [2, 3, 4, 5, 6]
  for (var c = 0; c < CONTROL_COLS.length; c++) cols.push(CONTROL_COLS[c][1], CONTROL_COLS[c][2])
  ...
  for (var r = 0; r < data.length; r++) {
    ...
    for (var ci = 0; ci < CONTROL_COLS.length; ci++) {
      ...
      if (status !== 'AL DIA' && status !== 'POR VENCER') continue
      ...
      var item = {
        ...
        restan: diasRest,
        aviso: dias,
        seria: diasRest <= dias ? 'POR VENCER' : 'AL DIA',
        actual: status,
      }
