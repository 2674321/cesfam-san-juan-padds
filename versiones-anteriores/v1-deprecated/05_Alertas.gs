...
function mostrarAlertas() {
  ...
  ss.toast('Generando alertas…', '', 1)
  var sh = ss.getSheetByName('Pacientes')
  if (!sh) { ss.toast('Hoja Pacientes no encontrada', '', 3); return }

  var lr = sh.getLastRow()
  if (lr < 4) { ss.toast('Sin datos de pacientes', '', 2); return }

  var cols = [2, 3, 4, 5, 6, 109]
  for (var c = 0; c < CONTROL_COLS.length; c++) {
    cols.push(CONTROL_COLS[c][1], CONTROL_COLS[c][2])
  }
  ...
  for (var r = 0; r < data.length; r++) {
    ...
    for (var ci = 0; ci < CONTROL_COLS.length; ci++) {
      ...
      if (ec === undefined || ec === null) continue
      if (idx[ec] === undefined) continue
      var status = String(row[idx[ec]] || '').trim()
      if (status === 'VENCIDO') { cc.v++ } else if (status === 'POR VENCER') { cc.pv++ } else if (status === 'PENDIENTE') { cc.p++ } else { continue }

      var paramKey = map[0].toUpperCase()
      var mesesParam = params[paramKey] || 12
      ...
      if (status === 'PENDIENTE') {
        pendientes.push({ label: label, fila: fila, control: map[0], razon: 'Sin fecha', tooltip: map[0] + ': sin fecha registrada · Máx ' + mesesParam + ' meses' })
        continue
      }
      ...
      if (status === 'VENCIDO') {
        ...
        vencidos.push({ label: label, fila: fila, control: map[0], razon: ago, tooltip: 'Último: ' + fechaStr + ' · Máx: ' + mesesParam + ' meses · Excedió por ' + diffMonths + ' meses' })
        ...
        porVencer.push({ label: label, fila: fila, control: map[0], razon: restanTexto, tooltip: 'Último: ' + fechaStr + ' · Restan ' + restan + ' días · Aviso: ' + diasAviso + ' días · Máx: ' + mesesParam + ' meses' })
      }
    }
