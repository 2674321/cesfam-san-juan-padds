...
function crearFormularioBase() {
  ...
  sh.getRange(2, 1)
    .setValue('Los formularios llegan automáticamente. Revise los datos, corrija si es necesario, luego use el menú: Aprobar (se copia a Pacientes y la fila se elimina de recepción) o Rechazar (la fila se elimina).')
...
function aprobarFormularios() {
  ...
  try {
    ...
    // y Rechazados, de abajo hacia arriba para no desalinear índices
...
function _batchCopiarFormularios(formRows) {
  ...
  // Limpieza: elimina filas extras vacías que puedan haber quedado en
  // Pacientes (solo filas 100% vacías, sin datos ni fórmulas)
