...
function _refrescarFormatoCondicional(sh, lr, lc) {
  ...
  for (var i = 0; i < statusCols.length; i++) {
    ...
    if (col === COL.V_RECETA) {
      rules.push(SpreadsheetApp.newConditionalFormatRule()
        .whenTextEqualTo('V. RECETA').setBackground('#c8e6c9').setFontColor('#2e7d32')
        .setRanges([rng]).build())
    }
  }
  ...
  var GRUPOS = [
    {
      ...
      items: [
        ...
        ['Vigencia recetas controladas', 3, 'Vigencia de las recetas controladas (columnas 112-113). Las recetas de medicamentos controlados (ej: clonazepam) suelen cubrir hasta 3 meses de tratamiento. El estado al día se muestra como "V. RECETA".'],
      ],
    },
    {
...
function crearDashboard() {
  ...
  var demo = [
    ...
    [5, 2, 'Pendiente',          cf(P + SEXO + '4:' + SEXO, 'PENDIENTE'), TG],
    [7, 2, 'Edad Promedio',      '=ROUND(AVERAGE(' + P + EDAD + '4:' + EDAD + ')' + SEP + ' 0)', GREEN],
    [9, 1, 'Electrodependiente', cf(P + colToLetter(29) + '4:' + colToLetter(29), 'SI'), '#6a1b9a'],
  ]
  ...
  for (var v = 0; v < CONTROL_COLS.length; v++) {
    var vr = R + vigRows
    var ec = CONTROL_COLS[v][2]
    if (!ec || ec > lc) continue
    var cl2 = colToLetter(ec)
    ...
    sh.getRange(vr, 1).setValue(nm2)
      ...
      .setBorder(true, true, true, true, true, true, '#c8e6c9', BS)
    var sts = [_vigenteReceta(nm2, 'AL DIA'), 'POR VENCER', 'VENCIDO', 'PENDIENTE', 'N/A']
  ...
  var ar = R
  var stCols = CONTROL_COLS.map(function(c) { return c[2] }).filter(function(x) { return x && x <= lc })
