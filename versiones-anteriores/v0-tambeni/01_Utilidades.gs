...
// Edad en años cumplidos desde una fecha de nacimiento (col I / F. NACIMIENTO → col J / EDAD)
...
// Borra SOLO filas 100% vacías (sin datos y sin fórmulas) desde desdeFila
// hacia abajo, de abajo hacia arriba. Nunca borra hojas ni filas con
// contenido. Devuelve cuántas filas eliminó.
...
// Normaliza valores de SEXO (col 7) y SEXO C. (col 17) a F / M / PENDIENTE.
// Acepta variantes escritas a mano (HOMBRE, MUJER, MASCULINO, FEMENINO, con
// minúsculas, etc.). Devuelve null si el valor no es reconocido (no se toca).
...
// Las recetas controladas usan "V. RECETA" como estado vigente (no "AL DIA").
function _vigenteReceta(nombre, estado) {
  if (estado !== 'AL DIA') return estado
  if (String(nombre || '').toUpperCase().indexOf('RECETA') !== -1) return 'V. RECETA'
  return estado
}
