...
// Restaura una hoja de sistema a su estado "recién creada" (sin borrarla):
// deshace merges, limpia contenido/formato, filas/columnas congeladas, filtros
// y reglas de formato condicional. Permite reutilizar la misma pestaña en vez
// de borrar y recrear (evita saltos de pestaña y pérdida de configuración).
function _resetHojaSistema(sh) {
  try {
    var _mrs = sh.getRange(1, 1, sh.getMaxRows(), sh.getMaxColumns()).getMergedRanges()
    for (var _mi = 0; _mi < _mrs.length; _mi++) {
      try { _mrs[_mi].breakApart() } catch(eB) {}
    }
  } catch(e1) {}
  try { sh.clear() } catch(e2) {}
  try { sh.setFrozenRows(0) } catch(e3) {}
  try { sh.setFrozenColumns(0) } catch(e4) {}
  try { var _fz = sh.getFilter(); if (_fz) _fz.remove() } catch(e5) {}
  try { sh.setConditionalFormatRules([]) } catch(e6) {}
}

// Estructura de Pacientes ya aplicada (versión + dimensiones de la hoja).
// Permite que una segunda ejecución del formateo completo omita reconstruir
// lo que no cambió (validaciones, CF, anchos, secciones, filtro…). El
// resultado final es idéntico al de una reconstrucción.
var _ESTRUCTURA_KEY = '_PADDS_ESTRUCTURA_PAC'
var _ESTRUCTURA_VERSION = 'padstr_v1'

function _claveEstructura(lr, lc) {
  return _ESTRUCTURA_VERSION + '|' + lr + '|' + lc
}

function _estructuraAplicada(lr, lc) {
  try {
    return PropertiesService.getScriptProperties().getProperty(_ESTRUCTURA_KEY) === _claveEstructura(lr, lc)
  } catch (e) { return false }
}

function _marcarEstructura(lr, lc) {
  try { PropertiesService.getScriptProperties().setProperty(_ESTRUCTURA_KEY, _claveEstructura(lr, lc)) } catch (e) {}
}

function _repintarSinConfirmar(sh, ss, force) {
  ...
  if (rows < 3) {   ss.toast(HOJA_PAC + ' sin datos para formatear', 'Pacientes', 4); return null }
  ...
  // Cabecera: solo se reescribe si cambió respecto a la plantilla (comparación
  // en memoria; un formateo repetido no vuelve a escribir lo mismo).
  var headerDirty = data[2].length !== currentCols
  if (!headerDirty) {
    for (var _hc2 = 0; _hc2 < currentCols; _hc2++) {
      if (String(data[2][_hc2] || '') !== String(_hdr[_hc2] || '')) { headerDirty = true; break }
    }
  }
  if (headerDirty) sh.getRange(3, 1, 1, currentCols).setValues([_hdr])

  // Datos: solo se escriben de vuelta si alguna fila quedó corta respecto a la
  // plantilla (si el contenido ya es idéntico, no se reescribe nada).
  var padCount = 0
  ...
  if (rows >= 4 && padCount > 0) {
    var dataRows = data.slice(3)
    sh.getRange(4, 1, dataRows.length, currentCols).setValues(dataRows)
  }
  aplicarFormatoFuncional(sh, rows, currentCols, force)
  ponerTooltipsPacientes()
  if (_migroIns) { try { crearDashboard() } catch(eD) {} }
  return { data: data.slice(3), lc: currentCols }
}

function _restaurarVisualSinConfirmar(sh, ss, force, dataMem, lcMem) {
  ...
  lc = sh.getLastColumn()
  _aplicarFormatoVisual(sh, lr, lc, force)
  try { recalcularTodo(dataMem, lcMem) } catch(e) {}
}
...
function _formatearDatosSinConfirmar(sh, ss, force) {
  ...
  var errores = []
  var mem = null
  try { mem = _repintarSinConfirmar(sh, ss, force) || null }
  ...
  try { _restaurarVisualSinConfirmar(sh, ss, force, mem && mem.data, mem && mem.lc) }
...
function repararFormatoPacientes() {
  ...
  try {
    ...
    var _merges = sh.getRange(1, 1, _maxR, _maxC).getMergedRanges()
  ...
  try { _formatearDatosSinConfirmar(sh, ss, true); pasos.push('✔ Diseño completo re-aplicado') }
...
function aplicarFormatoFuncional(sh, lr, lc, force) {

  if (force || !_estructuraAplicada(sh, lr, lc)) {
  ...
  _aplicarValidaciones(sh, sh.getMaxRows(), lc)
  ...
  if (lr >= 4) {
    ...
    for (var fi = 0; fi < _FECHAS_VA.length; fi++) {
      ...
      if (fc2 <= lc && _vacDef.indexOf(fc2) < 0) sh.getRange(4, fc2, sh.getMaxRows() - 3, 1).setDataValidation(dateVal)
    }
    ...
    for (var li = 0; li < libres.length; li++) {
      if (libres[li] <= lc) sh.getRange(4, libres[li], sh.getMaxRows() - 3, 1).setDataValidation(null)
    }
...
function _aplicarFormatoVisual(sh, lr, lc, force) {
  if (force || !_estructuraAplicada(sh, lr, lc)) {
  ...
  sh.getRange('E2').setValue('Filtros ▾').setFontStyle('italic')
    ...
    .setNote('Elige en F2 (una celda) qué sección mostrar temporalmente. Las demás columnas se ocultan; "SECCIONES" restaura. El buscador (B2), el filtro de estado (G2) y esta fila siempre quedan visibles.')
  var listaF = ['SECCIONES']
  for (var _lf = 0; _lf < PAC_SECCIONES.length; _lf++) {
    if (PAC_SECCIONES[_lf].ini <= lc) listaF.push(PAC_SECCIONES[_lf].nombre)
  }
  sh.getRange('F2').setValue('SECCIONES')
  sh.getRange('F2').setDataValidation(
    SpreadsheetApp.newDataValidation().requireValueInList(listaF, true)
      .setHelpText('Elige qué sección mostrar. Las demás columnas quedan ocultas temporalmente. "SECCIONES" restaura todo.')
      .build())
    .setFontWeight('bold').setFontColor('#1E293B').setFontSize(11).setHorizontalAlignment('center')
    .setNote('Mostrar sección: elige aquí qué sección ver (las demás columnas se ocultan temporalmente). "SECCIONES" restaura la vista completa. El buscador (B2) y esta fila siempre quedan visibles.')

  sh.getRange('G2').setValue('ESTADOS')
  sh.getRange('G2').setDataValidation(
    SpreadsheetApp.newDataValidation().requireValueInList(['ESTADOS'].concat(PAC_VALIDACIONES[6]), true)
      .setHelpText('Filtra los pacientes por estado de vigencia. "ESTADOS" restaura. Se combina con el buscador (B2).')
      .build())
    .setFontWeight('bold').setFontColor('#1E293B').setFontSize(11).setHorizontalAlignment('center')
    .setNote('Filtro por estado: elige aquí qué estado ver ("ESTADOS" restaura). Se combina con el buscador de B2 y el contador D2 se actualiza solo.')
  ...
  _marcarEstructura(sh, lr, lc)
  }
  // Las reglas de formato condicional se re-sincronizan en cada pasada (con
  // comparación), aunque la reconstrucción visual se haya omitido.
  try { _refrescarFormatoCondicional(sh, lr, lc, true) } catch(e) {}
}
...
function _refrescarFormatoCondicional(sh, lr, lc, skipDVs) {
  if (lr < 4) return
  if (!skipDVs) _aplicarValidaciones(sh, lr, lc)
  ...
  // Re-sincronización: se escribe solo si la lista existente difiere de la
  // estándar (reglas extras como el resaltado de fila agregado después del
  // último formateo se eliminan, igual que con la reconstrucción completa).
  // Huella dual: en el emulador (r.d) y en la API real (criterios y rangos),
  // porque JSON.stringify de un objeto nativo real devuelve {} y comparar
  // solo por longitud no detecta cambios de contenido con el mismo nº de reglas.
  try {
    var _huellaCF = function(r) {
      try { if (r && r.d !== undefined) return JSON.stringify(r.d || {}) } catch (eH) {}
      var a = []
      try {
        var rgs = r.getRanges()
        var tallas = []
        for (var z = 0; z < rgs.length; z++) {
          try { tallas.push(rgs[z].getA1Notation()) } catch (eZ) { tallas.push('R' + rgs[z].getRow() + 'C' + rgs[z].getColumn()) }
        }
        a.push(tallas.join(';'))
      } catch (eR) { a.push('') }
      try {
        var bc = r.getBooleanCriteria()
        var ct = '', cvs = []
        try { ct = bc.getCriteriaType() } catch (eC) {}
        try { cvs = (bc.getCriteriaValues ? bc.getCriteriaValues() : (bc.criteriaValues || [])) || [] } catch (eC2) {}
        a.push(ct, JSON.stringify(cvs))
      } catch (eB) { a.push('', '[]') }
      try { a.push(String(r.getBackground() || '')) } catch (eA) { a.push('') }
      try { a.push(String(r.getFontColor() || '')) } catch (eG) { a.push('') }
      try { a.push(String(r.getBold == null ? '' : r.getBold())) } catch (eBd) { a.push('') }
      try { a.push(String(r.getStrikethrough == null ? '' : r.getStrikethrough())) } catch (eSt) { a.push('') }
      return a.join('|')
    }
    var _existentes = []
    try { _existentes = sh.getConditionalFormatRules().map(_huellaCF) } catch (eGet) {}
    var _nuevas = rules.map(_huellaCF)
    var _iguales = _existentes.length === _nuevas.length
    if (_iguales) {
      for (var _ri = 0; _ri < _nuevas.length; _ri++) {
        if (_existentes[_ri] !== _nuevas[_ri]) { _iguales = false; break }
      }
    }
    if (!_iguales) sh.setConditionalFormatRules(rules)
  } catch(e) { try { sh.setConditionalFormatRules(rules) } catch(e2) {} }
}

// ─── SECTOR Y ESTADO (colorear por valor) ─────────────────────────────────

function _aplicarColorPorValor(sh, lr, col, coloresMap, dataMem) {
  ...
  var rows = lr - 3
  var data = dataMem || sh.getRange(4, col, rows, 1).getValues()
  var bgs = [], fgs = []
  for (var r = 0; r < rows; r++) {
    var v = String((dataMem ? dataMem[r][col - 1] : data[r][0]) || '').trim().toUpperCase()
    var m = coloresMap[v]
    bgs.push([m ? m[0] : null])
...
function colorearSector(sh, lr, dataMem) { _aplicarColorPorValor(sh, lr, 2, _SECTOR_COLORS, dataMem) }
function colorearEstado(sh, lr, dataMem) { _aplicarColorPorValor(sh, lr, 6, _ESTADO_COLORS, dataMem) }

// ─── COLOREAR VIGENCIA POR FECHA (un color por fecha) ──────────────────────

function colorearFechas(sh, lr, dataMem) {
  if (!sh) sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(HOJA_PAC)
  if (!sh) return
  if (!lr) lr = sh.getLastRow()
  if (lr < 4) return
  ...
  var vitalCol = dataMem ? null : sh.getRange(4, COL.VITAL, rows, 1).getValues()
  ...
  for (var fi = 0; fi < _FECHAS_COLOR.length; fi++) {
    ...
    var src = dataMem ? null : rng.getValues()
    var bgs = [], fgs = [], fmts = [], wts = []
    for (var r = 0; r < rows; r++) {
      var fallecido = dataMem
        ? String(dataMem[r][COL.VITAL - 1] || '').trim() === 'FALLECIDO'
        : String(vitalCol[r][0] || '').trim() === 'FALLECIDO'
      var st = fallecido ? 'N/A' : _estadoFecha(dataMem ? dataMem[r][fc - 1] : src[r][0], meses, diasAviso)
...
function crearParametros() {
  ...
  var sh = old || ss.insertSheet('Parámetros')
  if (old) _resetHojaSistema(sh)
...
function crearInstrucciones() {
  ...
  var sh = old || ss.insertSheet('Referencia Columnas')
  if (old) _resetHojaSistema(sh)
...
function crearDashboard() {
  ...
  var sh = old || ss.insertSheet('Dashboard')
  if (old) _resetHojaSistema(sh)
  ...
  try {
    ...
    for (var si = 0; si < sheets.length; si++) {
      ...
      for (var ri = 0; ri < existing.length; ri++) {
        ...
        try {
          // API real: los criterios se leen con getCriteriaValues(); el
          // acceso plano bc.criteriaValues solo existe en el emulador. Sin
          // esto, hasResalte nunca es true en Sheets real y se duplica la
          // regla en cada ejecución de configurarResaltadoFila.
          var bc = existing[ri].getBooleanCriteria()
          if (bc) {
            var cVals = []
            try { cVals = (bc.getCriteriaValues ? bc.getCriteriaValues() : (bc.criteriaValues || [])) || [] } catch(eCv) {}
            if (cVals.length && String(cVals[0] || '').indexOf('_Resalte') !== -1) hasResalte = true
          }
        } catch(e3) {}
