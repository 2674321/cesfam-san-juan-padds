...
// Pre-formateo de filas de datos (CF y DV del rango). Las editadas fuera de
// este rango se reponen al editar (_ingFormatearFila); ver informe Fase 5.
var _ING_PREFILAS = 500

// Fuentes de verdad de los dropdowns de Ingresos (DV, CF y textos usan estas).
var _ING_ACCIONES = ['INGRESA', 'NO INGRESA', 'PENDIENTE']
var _ING_ESTADOS = ['GESTIONADOS', 'PENDIENTES', 'EN ESPERA']

// Repone las validaciones de OBSERVACION/ESTADO si faltan (misma regla en
// formato completo y en edición de una fila; un solo lugar para la regla).
function _ingAplicarValidaciones(sh, desde, total, cols) {
  if (cols.accion) {
    try {
      if (sh.getRange(desde, cols.accion).getDataValidation() == null) {
        sh.getRange(desde, cols.accion, total, 1).setDataValidation(
          SpreadsheetApp.newDataValidation()
            .requireValueInList(_ING_ACCIONES, true).setAllowInvalid(true).build())
      }
    } catch (eDvA) {}
  }
  if (cols.estado) {
    try {
      if (sh.getRange(desde, cols.estado).getDataValidation() == null) {
        sh.getRange(desde, cols.estado, total, 1).setDataValidation(
          SpreadsheetApp.newDataValidation()
            .requireValueInList(_ING_ESTADOS, true).setAllowInvalid(true).build())
      }
    } catch (eDvE) {}
  }
}

// Cebra oficial (una sola implementación para formato completo y re-zebra).
function _ingPintarCebra(sh, desde, hasta, hr, lcF) {
  ...
  sh.getRange(desde, 1, n, lcF).setBackgrounds(bgArr)
}
...
function _ingAplicarFormato(sh, d) {
  ...
  _ingPintarCebra(sh, desde, hasta, hr, lcF)
  ...
  _ingAplicarValidaciones(sh, desde, total, cols)
...
function _ingConstruirPanel(sh, d) {
  ...
  var mergesPrev = []
  try { mergesPrev = sh.getRange(1, 1, sh.getMaxRows(), sh.getMaxColumns()).getMergedRanges() } catch (eM0) {}
...
function _ingFormatearFila(sh, row, numRows, d) {
  ...
  // Filas fuera del rango pre-formateado (hr + _ING_PREFILAS): reponer las
  // validaciones de OBSERVACION/ESTADO que _ingAplicarFormato solo agrega en
  // ese rango, para que toda fila editada quede igual que las del rango.
  if (row > hr + _ING_PREFILAS) {
    _ingAplicarValidaciones(sh, row, maxFilas, cols)
  }
...
function _onEditIngresosImpl(e) {
  ...
  try {
    _optsRefsCache = null
    _refrescarFormatoCondicional(pac, pac.getLastRow(), pac.getLastColumn(), true)
  } catch (eCF4) {}
...
function _ingReBandear(sh, d, desdeFila) {
  ...
  _ingPintarCebra(sh, desdeFila, lr, hr, lcF)
