...
// Edad detallada con meses ("75 años · 3 meses", "8 meses"). Sin fecha de
// nacimiento válida usa la edad en años almacenada (o devuelve '').
function _edadConMeses(fechaNac, edadAnios) {
  var d = _parseDate(fechaNac)
  if (d) {
    var hoy = new Date()
    var totalMeses = (hoy.getFullYear() - d.getFullYear()) * 12 + (hoy.getMonth() - d.getMonth())
    if (hoy.getDate() < d.getDate()) totalMeses--
    if (totalMeses >= 0) {
      var anios = Math.floor(totalMeses / 12)
      var meses = totalMeses % 12
      if (anios > 0) {
        var tA = anios + (anios === 1 ? ' año' : ' años')
        return meses > 0 ? tA + ' · ' + meses + ' mes' + (meses === 1 ? '' : 'es') : tA
      }
      return meses + ' mes' + (meses === 1 ? '' : 'es')
    }
  }
  return edadAnios ? edadAnios + ' años' : ''
}
...
function _unmergeQueCruzaFila(sh, n) {
  try {
    var mrs = sh.getRange(1, 1, sh.getMaxRows(), sh.getMaxColumns()).getMergedRanges()
