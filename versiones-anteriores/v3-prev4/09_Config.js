...
function onOpen() {
  ...
  ui.createMenu('🏠 Inicio')
    ...
    .addSeparator()
    .addItem('🔎 Buscar paciente', 'buscarPacienteGlobal')
    .addItem('📄 Ficha', 'verFichaPacienteV4')
    .addItem('🚨 Alertas', 'mostrarAlertasV4')
    .addItem('✅ Tareas', 'mostrarTareas')
    .addSeparator()
    .addItem('🛡️ Auditoría', 'mostrarAuditoriaDatos')
    .addItem('📚 Guía', 'mostrarGuiaColumnas')
    .addSeparator()
  ...
  ui.createMenu('📅 Agenda')
    ...
    .addItem('🔎 Buscar en agenda', 'buscarPaciente')
    .addSeparator()
  ...
  ui.createMenu('🩺 Pacientes')
    ...
    .addItem('📄 Ficha con vigencias', 'verFichaPaciente')
    .addSeparator()
    .addSubMenu(ui.createMenu('🛠️ Datos')
      ...
      .addSeparator()
      .addItem('🎨 Formatear hoja (Pacientes)', 'formatearDatos')
      ...
      .addSeparator()
      .addItem('⚠️ Eliminar paciente', 'eliminarPaciente'))
    .addItem('🔄 Recalcular vigencias', 'recalcularTodo')
    .addToUi()

  ui.createMenu('📥 Ingresos')
    .addItem('🎨 Formatear hoja (Ingresos)', 'corregirYMejorarIngresos')
    .addSeparator()
  ...
  ui.createMenu('🔧 Herramientas')
    .addItem('♻️ Restablecer', 'restablecerPADDS')
    .addSeparator()
    .addSubMenu(ui.createMenu('📊 Paneles')
...
function mostrarAyudaRapida() {
  var css = '<style>' +
    ...
    '.sub{font-size:11.5px;color:#475569;margin:0 0 12px}' +
    ...
    '.card>summary::after{content:"▾";margin-left:auto;color:#64748B;font-size:12px;transition:transform .15s}' +
  ...
  var secs = [
    { c: '#B45309', t: '📅 Agenda', open: false,
      h: '<p><b>¿Qué es?</b> Planifica visitas domiciliarias por semana; cada profesional tiene su columna y el horario es fijo de 8:00 a 16:00.</p>' +
         ...
         '<p><b>Navegar:</b> <span class="k">🎯 Ir a hoy</span>, <span class="k">📅 Ir a fecha</span> o <span class="k">🔎 Buscar en agenda</span>.</p>' },
    { c: '#0369A1', t: '🩺 Pacientes', open: true,
      h: '<p><b>¿Qué es?</b> La ficha central: datos personales, patologías, controles y cuidador (111 campos por secciones de color).</p>' +
         ...
         '<p><b>Mostrar secciones:</b> a la derecha del buscador, en <b>F2</b> (una celda) un <b>menú desplegable</b> elige qué sección ver: las demás columnas se ocultan temporalmente y se ven solo las de esa sección. <b>"SECCIONES"</b> restaura todo. La zona del buscador (columnas 1-17) siempre queda visible.</p>' +
         ...
         '<div class="tip">💡 Plazos y días de aviso: 🔧 Herramientas → 📊 Paneles → ⚙️ Parámetros.</div>' },
    ...
    { c: '#475569', t: '🔧 Herramientas', open: false,
      h: '<p><b>♻️ Restablecer:</b> al primer uso, crea todas las hojas del sistema (Parámetros, Ingresos, Recepción, Dashboard, Referencia Columnas, Agenda, _Resalte) y formatea ' + HOJA_PAC + ' de una sola vez.</p>' +
    ...
    { c: '#B91C1C', t: '🚨 Alertas', open: false,
      h: '<p>Panel visual de pacientes con controles <b>VENCIDOS</b>, <b>POR VENCER</b>, <b>PENDIENTES</b> y <b>PRIORITARIOS</b>. Haz clic en un paciente para ir a su fila.</p>' +
         '<p class="men">Acceso: 🏠 Inicio → 🚨 Alertas (o ✅ Tareas).</p>' },
    { c: '#475569', t: '👨‍⚕️ Créditos', open: false,
      h: '<p><b>Sistema PADDS</b> — Planilla de Atención Domiciliaria para CESFAM.</p>' +
         ...
         '<p><b>Versión:</b> V5.6 · <b>Última actualización:</b> 14/08/2026</p>' },
  ]
...
function acercaDelSistema() {
  var html = '<html><head><base target="_top"><style>' +
    ...
    '.f{font-size:11px;color:#64748B;text-align:center;margin-top:14px}' +
    '</style></head><body>' +
    '<h2>Sistema PADDS</h2>' +
    '<div class="ver">Planilla de Atención Domiciliaria para CESFAM · V5.6</div>' +
    ...
    '<div class="m"><div class="ic" style="background:#0F766E">📅</div><div><b>Última actualización</b><span>14/08/2026 · versión 5.5</span></div></div>' +
...
function restablecerPADDS() {
  ...
  var resp = ui.alert('♻️ Restablecer PADDS',
  ...
  if (resp !== ui.Button.YES) return
  if (!shPac) {
    ui.alert('♻️ Restablecer PADDS', 'No se puede restablecer sin la hoja "' + HOJA_PAC + '".\n' + avisoPac, ui.ButtonSet.OK)
    return
  }
  ...
  ss.toast('Restablecer PADDS: ' + ok + ' pasos OK' + (fail ? ', ' + fail + ' con error' : ''), 'PADDS', 8)
  ui.alert('♻️ Restablecer PADDS',
    (fail ? '⚠️ Algunos pasos fallaron:\n\n' : '✅ Restablecimiento completado:\n\n') + resumen.join('\n'),
    ui.ButtonSet.OK)
}

// ⚠️ Alias de compatibilidad: el menú ya usa restablecerPADDS. Se conserva para
// no romper autorizaciones/triggers o referencias externas históricas a este nombre.
function inicializarPADDS() {
  return restablecerPADDS()
}
