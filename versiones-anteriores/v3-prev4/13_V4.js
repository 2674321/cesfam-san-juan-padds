// ════════════════════════════════════════════════════════
// ÍNDICE ▏13_V4.gs │ PADDS V5.6 — capa de producto
//
...
function _uiV4Shell(titulo, bodyHtml, ancho) {
  var html = '<!DOCTYPE html><html><head><base target="_top"><style>' + _uiCss() + _uiExtCss() + '</style></head><body>' +
    bodyHtml + _uiScriptComun() + '</body></html>'
  return HtmlService.createHtmlOutput(html).setTitle(titulo).setWidth(ancho || _V4_ANCHO)
}

// ─── DIGEST DE TEXTOS DE UI (regresión Unicode) ─────────
// Devuelve los textos visibles clave de la capa V4 para que
// ejecutarPruebas verifique que no contienen mojibake.
function digestUIStrings() {
  return [
    '🧭 Centro de control',
    'viernes · PADDS V5.6',
    '🔎 Buscar paciente',
...
function _datosCentroControl() {
  var out = { pacientes: 0, vigentes: 0, egresados: 0, alertasC: 0, alertasI: 0, formPend: null, hasPac: false, alertas: null }
  var ss = SpreadsheetApp.getActiveSpreadsheet()
  var sh = ss.getSheetByName(HOJA_PAC)
  if (sh && sh.getLastRow() >= 4) {
    out.hasPac = true
    var d = sh.getRange(4, 2, sh.getLastRow() - 3, 7).getValues()
    for (var r = 0; r < d.length; r++) {
      var es = String(d[r][4] || '').trim()
      out.pacientes++
      if (es === 'VIGENTE') out.vigentes++
      else if (es === 'EGRESO' || es === 'EGRESO POR ALTA' || es === 'ALTA') out.egresados++
    }
  }
  try { var al = _calcularAlertas(); if (al) { out.alertasC = al.vencidos.length; out.alertasI = al.porVencer.length; out.alertas = al } } catch (eA) {}
  try { var est = _obtenerEstadisticas(); if (est) out.formPend = est.pend } catch (eE) {}
  try {
    var shIn = ss.getSheetByName('Ingresos')
    if (shIn) {
      var dIn = _ingDetectarColumnas(shIn)
      if (dIn && shIn.getLastRow() >= dIn.headerRow + 1) {
        var hrIn = dIn.headerRow
        var nRows = shIn.getLastRow() - hrIn
        var cEst = dIn.cols.estado || 0
        var cRun = dIn.cols.run || 0
        if (cEst) {
          // "Por gestionar": filas pendientes (ESTADO distinto de GESTIONADOS),
          // sin contar vacías reales ni encabezados. Una sola lectura de 2 columnas.
          var vEst = shIn.getRange(hrIn + 1, cEst, nRows, 1).getValues()
          var vRun = cRun ? shIn.getRange(hrIn + 1, cRun, nRows, 1).getValues() : null
          var pend = 0
          for (var i = 0; i < nRows; i++) {
            var st = String(vEst[i][0] || '').trim()
            var rn = vRun ? String(vRun[i][0] || '').trim() : ''
            if (st === 'GESTIONADOS') continue
            if (!st && !rn) continue
            pend++
          }
          out.ingresosN = pend
...
function abrirCentroControl() {
  ...
  b.push('<div class="sub">' + new Date().toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long' }) + ' · PADDS V5.6</div>')
  ...
  var nAlertas = 0, nTareas = 0
  try { nAlertas = d.alertasC + d.alertasI } catch (eA) {}
  try { var _tl = _tareasDesdeAlertas(d.alertas); if (_tl) nTareas = _tl.length } catch (eT) {}

  b.push('<div class="grid2">' +
    '<button class="btn btn-teal btn-lg" onclick="google.script.run.buscarPacienteGlobal()">🔎 Buscar<span class="btn-sub">paciente</span></button>' +
    '<button class="btn btn-lg" onclick="google.script.run.verFichaPacienteV4()">📄 Ficha<span class="btn-sub">paciente</span></button>' +
    '<button class="btn btn-sec btn-lg" onclick="google.script.run.mostrarAlertasV4()">🚨 Alertas' + (nAlertas ? ' <span class="bdg bdg-err">' + nAlertas + '</span>' : '') + '<span class="btn-sub">controles</span></button>' +
    '<button class="btn btn-sec btn-lg" onclick="google.script.run.mostrarTareas()">✅ Tareas' + (nTareas ? ' <span class="bdg bdg-warn">' + nTareas + '</span>' : '') + '<span class="btn-sub">acciones</span></button>' +
    '</div>')

  b.push('<div class="drop" id="drop-mas">' +
    '<button class="btn btn-sec btn-lg btn-block" data-drop="drop-mas" aria-haspopup="true" aria-expanded="false">⋯ Más</button>' +
    '<div class="drop-panel">' +
    '<button class="btn btn-sec" onclick="google.script.run.mostrarAuditoriaDatos()">🛡️ Auditoría</button>' +
    '<button class="btn btn-sec" onclick="google.script.run.mostrarGuiaColumnas()">📚 Guía de columnas</button>' +
    '</div></div>')

  b.push('<div class="card"><div class="card-t">Hojas</div><div class="grid2">' +
    '<button class="btn btn-sec" onclick="google.script.run._irA(\'Pacientes\')">👥 Pacientes</button>' +
    '<button class="btn btn-sec" onclick="google.script.run._irA(\'Recepción Formulario Profesional\')">📥 Recepción</button>' +
    '<button class="btn btn-sec" onclick="google.script.run._irA(\'Ingresos\')">📋 Ingresos</button>' +
    '<button class="btn btn-sec" onclick="google.script.run.irAHoy()">📅 Agenda</button>' +
    '<button class="btn btn-sec" onclick="google.script.run.abrirDashboardV4()">📊 Dashboard</button>' +
    '</div></div>')

  if (d.formPend != null) b.push('<div class="sub">📥 Recepción: <b>' + d.formPend + '</b> formulario(s) pendiente(s)</div>')
  if (d.ingresosN != null) b.push('<div class="sub">📋 Ingresos: <b>' + d.ingresosN + '</b> registro(s) por gestionar</div>')
  _mostrar(b)
}

function _mostrar(partes) {
  SpreadsheetApp.getUi().showSidebar(_uiV4Shell('Centro de control · PADDS V5.6', partes.join('')))
}
...
'.it{background:#fff;border:1px solid #E2E8F0;border-radius:8px;padding:10px;margin-bottom:6px}' +
'.it .nom{font-size:14px;font-weight:600;color:#1E293B}' +
'.it .det{font-size:12px;color:#64748B;margin:2px 0;line-height:1.5}' +
'.it .rut{font-family:Consolas,monospace;color:#0F766E}' +
'.it .ops{margin-top:6px;display:flex;gap:6px}' +
'.it-msg{padding:26px 12px;text-align:center}' +
'.it-msg .emoji{font-size:26px}' +
'.it-msg .t1{font-weight:600;color:#475569;margin-top:6px}' +
'.it-msg .t2{font-size:12px;color:#64748B;margin-top:4px}' +
'.btn-retry{margin-top:10px;display:inline-block}' +
'</style></head><body>' +
'<input type="text" id="q" aria-label="Buscar paciente por nombre, RUN, apellido o teléfono" placeholder="Nombre, RUN, apellido o teléfono..." autofocus>' +
'<div class="bar-info" id="cta"></div>' +
'<div id="res"><div class="it-msg"><div class="emoji">🔎</div><div class="t1">Escribe para buscar</div><div class="t2">Puedes usar nombre, RUN, teléfono o N° de paciente</div></div></div>' +
'<script>' +
...
'var ru=document.createElement("span");ru.className="rut";ru.textContent=_e(o.ru||"—");dt.appendChild(ru);' +
'if(o.ed){var ed=document.createElement("span");ed.textContent="  "+_e(o.ed)+" años";dt.appendChild(ed)}' +
'if(o.x){var sx=document.createElement("span");sx.textContent="  "+_e(o.x);dt.appendChild(sx)}' +
'if(o.s){var sec=document.createElement("span");sec.textContent="  📍"+_e(o.s);dt.appendChild(sec)}' +
...
'var b1=document.createElement("button");b1.className="btn btn-sm btn-teal";b1.textContent="Ficha";' +
'b1.onclick=function(){el.classList.add("it-sel");_uiPac(this,o);_uiFicha(b1,o.f)};ops.appendChild(b1);' +
'var b2=document.createElement("button");b2.className="btn btn-sm btn-sec";b2.textContent="Ir";' +
'b2.onclick=function(){el.classList.add("it-sel");_uiPac(this,o);_uiSel(b2,o.f)};ops.appendChild(b2);' +
'el.appendChild(ops);return el}' +
'function _err(){res.innerHTML=window.__errHtml||"";var rb=document.getElementById("retry");if(rb)rb.onclick=function(){_do()}}' +
'function _do(){' +
'var v=inp.value.trim();if(!v){res.innerHTML=window.__idleHtml;cta.textContent="";try{sessionStorage.removeItem("padds_q")}catch(x){}return}' +
'cta.textContent="Buscando...";try{sessionStorage.setItem("padds_q",v)}catch(x){}' +
'google.script.run.withSuccessHandler(function(r){' +
'cta.textContent=r.length+" resultado"+(r.length===1?"":"s");res.innerHTML="";' +
'if(!r.length){res.innerHTML=window.__emptyHtml;return}' +
'var fr=document.createDocumentFragment();for(var i=0;i<r.length;i++)fr.appendChild(_add(r[i]));res.appendChild(fr)' +
'}).withFailureHandler(function(e){res.innerHTML=window.__errHtml;var rb=document.getElementById("retry");if(rb)rb.onclick=function(){_do()};cta.textContent="Error de búsqueda"})._doBuscarPacientes(v)}' +
'inp.addEventListener("input",function(){clearTimeout(_t);_t=setTimeout(_do,250)});' +
'inp.addEventListener("keydown",function(e){if(e.key==="Enter"){clearTimeout(_t);_do()}else if(e.key==="Escape"){try{google.script.host.close()}catch(x){}}});' +
'window.__emptyHtml=\'<div class="it-msg"><div class="emoji">🔎</div><div class="t1">No se encontraron pacientes</div><div class="t2">Prueba por RUN, nombre o apellido</div></div>\';' +
'window.__errHtml=\'<div class="it-msg"><div class="emoji">⚠️</div><div class="t1">No se pudo completar la búsqueda</div><div class="t2">Revisa la conexión e intenta de nuevo</div><button class="btn btn-teal btn-sm btn-retry" id="retry">↻ Reintentar</button></div>\';' +
'window.__idleHtml=document.getElementById("res").innerHTML;' +
'var q0=null;try{q0=sessionStorage.getItem("padds_q")}catch(x){}' +
'if(q0){inp.value=q0;_do()}' +
'</script>' + _uiScriptComun() + '</body></html>'
  SpreadsheetApp.getUi().showSidebar(
    HtmlService.createHtmlOutput(html).setTitle('Buscador global · PADDS V5.6').setWidth(_V4_ANCHO)
  )
}
...
function _filaV4Html(vals, fila, lc) {
  ...
  var edadTxt = _edadConMeses(vals[8], edad) || '?'
  ...
  b.push('<div class="sub">' + _esc(run || 'Sin RUN') + ' · ' + _esc(edadTxt) + ' · ' + _esc(sexo || '—') + ' · Fila ' + fila + '</div>')
  ...
  b.push('<div style="display:flex;gap:6px;margin:10px 0 12px">' +
    '<button class="btn btn-teal btn-sm" onclick="_uiSel(this,' + fila + ')">Ir</button>' +
    '<button class="btn btn-sec btn-sm" onclick="_uiPdf(this,' + fila + ')">PDF</button>' +
  ...
  return '<!DOCTYPE html><html><head><base target="_top"><meta charset="UTF-8"><style>' + _uiCss() + _uiExtCss() + '</style></head><body>' + b.join('') + _uiScriptComun() + '</body></html>'
}

// ─────────────────────────────────────────────────────────
// ─── ALERTAS + TAREAS V4 ─────────────────────────────────

function _tareasDesdeAlertas(preR) {
  var r = preR || _calcularAlertas()
  if (!r) return []
  var t = []
...
function _tareasV4Html(preR) {
  var t = _tareasDesdeAlertas(preR)
  ...
  for (var i2 = 0; i2 < t.length; i2++) {
    ...
    b.push('<div class="card" style="margin-bottom:8px">' +
      ...
      '<button class="btn btn-sec btn-sm" data-f="' + ta.fila + '" data-n="' + encodeURIComponent(ta.label) + '" onclick="_uiPacF(this,\'sel\')">Ir</button></div>' +
      '</div>')
  }
  return b.join('')
}

function _alertasV4Html(preR) {
  var r = preR || _calcularAlertas()
  if (!r) return ''
  ...
  var secciones = [
    ['🚨 Prioritarios', r.urgentes, function(x) { return x.detalle }, 'CRITICA'],
    ['🔴 Vencidos', r.vencidos, function(x) { return x.control + ' · ' + x.razon }, 'CRITICA'],
    ['🟠 Por vencer', r.porVencer, function(x) { return x.control + ' · ' + x.razon }, 'IMPORTANTE'],
    ['🟡 Sin fecha', r.pendientes, function(x) { return x.control }, 'PENDIENTE'],
  ]
  for (var s = 0; s < secciones.length; s++) {
    var sec = secciones[s]
    if (!sec[1].length) continue
    var bd = _sevEstado(sec[3]).badge
    b.push('<div class="acc collapsed"><div class="acc-h">' + sec[0] + ' (' + sec[1].length + ')</div><div class="acc-c">')
    for (var i = 0; i < sec[1].length; i++) {
      var it = sec[1][i]
      var nomEnc = encodeURIComponent(it.label)
      b.push('<div class="card" style="margin:0 0 8px">' +
        '<div class="row-l"><span style="font-weight:600">' + _esc(it.label) + '</span>' +
        '<span class="badge ' + bd + '">' + sec[3] + '</span></div>' +
        '<div class="sub" style="margin:4px 0">' + _esc(sec[2](it)) + '</div>' +
        '<div class="row-l"><span class="dim">' + _esc(it.tooltip || '') + '</span>' +
        '<button class="btn btn-sec btn-sm" data-f="' + it.fila + '" data-n="' + nomEnc + '" onclick="_uiPacF(this,\'ficha\')">Ficha</button>' +
        '<button class="btn btn-sec btn-sm" data-f="' + it.fila + '" data-n="' + nomEnc + '" onclick="_uiPacF(this,\'sel\')">Ir</button></div>' +
        '</div>')
    }
    b.push('</div></div>')
  }
...
function _mostrarAlertasTareas(abrirTareas) {
  ...
  var body = '<div class="tabs">' +
    ...
    '<div id="tabA">' + _alertasV4Html(r) + '</div>' +
    '<div id="tabT" style="display:' + (abrirTareas ? '' : 'none') + '">' + _tareasV4Html(r) + '</div>' +
  ...
  SpreadsheetApp.getUi().showSidebar(_uiV4Shell('Alertas y tareas · PADDS V5.6', body))
}
...
function mostrarAuditoriaDatos() {
  ...
  var chipRow = []
  var clm = { '🔴': 'chip-err', '🟠': 'chip-warn', '🟡': 'chip-info' }
  for (var gx = 0; gx < grupos.length; gx++) {
    if (grupos[gx][1].length) chipRow.push('<span class="chip ' + clm[grupos[gx][0].slice(0, 1)] + '">' + grupos[gx][0].slice(2) + ' ' + grupos[gx][1].length + '</span>')
  }
  if (chipRow.length) b.push('<div style="margin-bottom:10px">' + chipRow.join(' ') + '</div>')
  var tieneAlgo = false
  for (var g = 0; g < grupos.length; g++) {
    ...
    for (var i = 0; i < Math.min(gr[1].length, 20); i++) {
      ...
      b.push('<div class="kv"><span class="k">' + _esc(it.label) + '<div class="dim">' + _esc(it.detalle) + '</div></span>' +
        '<span>' +
        '<button class="btn btn-sec btn-sm" data-f="' + it.fila + '" data-n="' + nomEnc + '" onclick="_uiPacF(this,\'ficha\')">Ficha</button>' +
        '<button class="btn btn-sec btn-sm" data-f="' + it.fila + '" data-n="' + nomEnc + '" onclick="_uiPacF(this,\'sel\')">📍 Ir</button>' +
        '</span></div>')
    }
  ...
  SpreadsheetApp.getUi().showSidebar(_uiV4Shell('Auditoría de datos · PADDS V5.6', b.join('')))
}
...
function mostrarGuiaColumnas() {
  ...
  b.push('<input type="text" id="gq" class="input" aria-label="Buscar columna o concepto" placeholder="Buscar columna o concepto..." style="margin-bottom:10px">')
  ...
  for (var c = 1; c < _COLUMNAS.length; c++) {
    ...
    b.push('<div class="acc" data-s="' + _esc(sec) + '" data-t="' + txt + '">' +
      '<div class="acc-h" role="button" tabindex="0" aria-expanded="false">' + _esc(String(c)) + ' · ' + _esc(col.name) + '</div>' +
      '<div class="acc-c">' +
  ...
  b.push('<script>' +
    ...
    'c.setAttribute("role","button");c.setAttribute("tabindex","0");c.setAttribute("aria-label","Filtrar por sección: "+s);' +
    'c.style.cursor="pointer";c.onclick=function(){var k="sec";if(filtros[k]===s){delete filtros[k];applyF();return}filtros[k]=s;applyF()};' +
    'c.addEventListener("keydown",function(e){if(e.key==="Enter"||e.key===" "){e.preventDefault();c.click()}});fz.appendChild(c)});' +
    ...
    'q.addEventListener("keydown",function(e){if(e.key==="Escape"){try{google.script.host.close()}catch(x){}}});' +
    '</script>')
  SpreadsheetApp.getUi().showSidebar(_uiV4Shell('Guía de columnas · PADDS V5.6', b.join('')))
}
