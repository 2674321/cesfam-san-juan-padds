...
// ─── MOTOR DE BÚSQUEDA ÚNICO (V5.2) ─────────────────────────────────────────
// El Buscador Global (buscarPacienteGlobal en 13_V4) es el ÚNICO buscador de
// pacientes del sistema. El buscador de la hoja (buscarEnPacientes) fue
// eliminado: su UI duplicaba al Global y su backend (_doBuscarPacientes) se
// conserva aquí como el motor único que consumen todas las interfaces.

// Índice de búsqueda en memoria (V5.5): lee Pacientes UNA vez y deja el
// resultado pre-normalizado en CacheService (60 s). Cada búsqueda posterior
// filtra/ordena el índice SIN volver a leer la hoja ni re-normalizar filas.
// Se invalida con _invalidaIndicePacientes() en cada escritura conocida
// (agregarPaciente, _actualizarEstadosFila, recalcularTodo, ordenarPacientes,
// limpiar/formatear y onEdit sobre filas de Pacientes) y por TTL como red.
// Si el cache falla, se reconstruye igual que antes (mismo resultado).

var _INDI_KEY = 'pads_ind_v1'

function _invalidaIndicePacientes() {
  try { CacheService.getScriptCache().remove(_INDI_KEY) } catch (e) {}
}

function _leerIndicePacientes() {
  try {
    var cHit = CacheService.getScriptCache()
    var hit = cHit.get(_INDI_KEY)
    if (hit) {
      var idxHit = JSON.parse(hit)
      if (idxHit && idxHit.length) return idxHit
    }
  } catch (e) {}
  ...
  var data = sh.getRange(4, 1, lr - 3, 13).getValues()
  var idx = []
  for (var r = 0; r < data.length; r++) {
    var row = data[r]
    var nombre = String(row[2] || '')
    var ap = String(row[3] || '')
    var ap2 = String(row[4] || '')
    var nFull = nombre + ' ' + ap + ' ' + ap2
    var runRaw = String(row[7] || '')
    var run = _norm(runRaw)
    var tel = String(row[11] || '')
    idx.push({
      f: r + 4,
      n: nFull,
      ru: runRaw,
      ...
      nn: _norm(nFull),
      an: _norm(ap),
      idn: _norm(String(row[0] || '')),
      run: run,
      runDig: run.replace(/[^a-z0-9]/g, ''),
      telDig: _norm(tel).replace(/[^a-z0-9]/g, '')
    })
  }
  if (idx.length) {
    try { CacheService.getScriptCache().put(_INDI_KEY, JSON.stringify(idx), 60) } catch (eP) {}
  }
  return idx
}
...
function _doBuscarPacientes(q) {
  var term = _norm(q)
  if (!term) return []
  var termDig = term.replace(/[^a-z0-9]/g, '')
  var idx = _leerIndicePacientes()
  ...
  for (var i = 0; i < idx.length; i++) {
    ...
    if (it.run === term || it.runDig === termDig) score = 0
    else if (it.idn === term) score = 1
    else if (it.nn.indexOf(term) === 0 || it.an.indexOf(term) === 0) score = 2
    else if (it.telDig === termDig || it.telDig.indexOf(termDig) === 0) score = 3
    else if (it.telDig.indexOf(termDig) !== -1) score = 5
    else if (it.runDig.indexOf(termDig) !== -1) score = 7
    else if (it.nn.indexOf(term) !== -1 || it.nn.indexOf(termDig) !== -1) score = 6
  ...
  res.sort(function (a, b) {
    if (a._sc !== b._sc) return a._sc - b._sc
    return a.nn < b.nn ? -1 : a.nn > b.nn ? 1 : 0
  })
  var out = []
  var top = Math.min(res.length, 30)
  for (var j = 0; j < top; j++) {
    var w = res[j]
    out.push({
      f: w.f,
...
// estado (columna F) no coincida con el dropdown de G2. ESTADOS/vacío = sin filtro.
function _aplicarFiltrosPac(sh) {
  ...
  var usando = term !== '' || (est !== '' && est !== 'ESTADOS')
  var lr = sh.getLastRow()
  if (lr < 4) return
  ...
  for (var r = 0; r < data.length; r++) {
    ...
    var rowMatch = est === '' || est === 'ESTADOS' || estadoRow === est
    if (rowMatch && term !== '') {
...
function aplicarFiltroSecciones(sh) {
  ...
  var todas = sel === '' || sel.toUpperCase() === 'SECCIONES'
...
function agregarPaciente() {
  ...
  if (lr >= 4) {
    sh.getRange(lr, 1, 1, lc).copyTo(sh.getRange(nr, 1, 1, lc), { formatOnly: true })
  }
...
function recalcularTodo(dataMem, lcMem) {
  var ss = SpreadsheetApp.getActiveSpreadsheet()
  var sh = ss.getSheetByName(HOJA_PAC)
  if (!sh) {   ss.toast('No se encontró la hoja ' + HOJA_PAC + '. Revisa que exista.', 'Pacientes', 4); return }
  var lr = dataMem ? dataMem.length + 3 : sh.getLastRow()
  ...
  var colIdx = {}
  // Índice directo (col - 1): la data siempre es de ancho completo (1..lc),
  // venga en memoria (dataMem) o se lea de la hoja en una sola llamada.
  for (var i = 0; i < cols.length; i++) colIdx[cols[i]] = cols[i] - 1

  var data = dataMem || sh.getRange(4, 1, lr - 3, lc).getValues()
  ...
  try { colorearSector(sh, lr, data) } catch(e) {}
  try { colorearEstado(sh, lr, data) } catch(e) {}
  try { colorearFechas(sh, lr, data) } catch(e) {}
  try { _colorearPrioridad(sh, lr, data) } catch(e) {}
  try { _colorearEdadPorEMPA(sh, lr, lc, data) } catch(e) {}
  try {
...
function _colorearPrioridad(sh, lr, dataMem) {
  ...
  var rows = lr - 3
  var data = dataMem || sh.getRange(4, COL.PRIORIDAD, rows, 1).getValues()
  var bgs = [], fgs = [], tBgs = [], tFgs = [], tWts = []
  var pmap = _PRIORIDAD_PMAP
  for (var r = 0; r < rows; r++) {
    var k = String((dataMem ? dataMem[r][COL.PRIORIDAD - 1] : data[r][0]) || '').trim().toUpperCase()
    var m = pmap[k] || null
    bgs.push([m ? m[0] : null])
...
function _colorearEdadPorEMPA(sh, lr, lc, dataMem) {
  if (lr < 4) return
  ...
  if (cN > lc) cN = lc
  if (c1 > lc) return
  var data = dataMem || sh.getRange(4, c1, r, cN - c1 + 1).getValues()
  var bgU = [], bgC = []
  for (var i = 0; i < r; i++) {
    var valC = empaCCol <= lc
      ? String((dataMem ? dataMem[i][empaCCol - 1] : data[i][empaCCol - c1]) || '').trim()
      : ''
    var valU = empaUCol <= lc
      ? String((dataMem ? dataMem[i][empaUCol - 1] : data[i][empaUCol - c1]) || '').trim()
      : ''
...
function formatearTelefonos(confirmado) {
  ...
  var data = sh.getRange(4, COL.TELEFONO, rows, 1).getValues()
  var col12 = []
  ...
  if (dirty12) sh.getRange(4, COL.TELEFONO, rows, 1).setValues(col12)
...
function formatearRUTPacientes(confirmado) {
  ...
  for (var ci = 0; ci < runCols.length; ci++) {
    ...
    var bgs = [], fgs = [], notas = [], notasDirty = false
    for (var i = 0; i < rows; i++) {
      ...
      notas.push([nota])
      if (String(notes[i][0] || '') !== nota) notasDirty = true
    }
    rng.setBackgrounds(bgs).setFontColors(fgs)
    if (notasDirty) rng.setNotes(notas)
  }
...
function verificarIntegridadRUN() {
  ...
  var html = '<html><head><base target="_top"><style>' + _uiCss() +
    '.row{padding:2px 0;border-bottom:1px solid #F1F5F9;font-size:12px}.row:last-child{border-bottom:none}' +
    '</style></head><body>' +
...
function _buildFichaSidebarHtml(headers, data, lc, autor, row) {
  ...
  var edadTxt = _edadConMeses(data[8], edad) || '?'
  ...
  p.push('.badge{display:inline-block;padding:2px 10px;border-radius:999px;font-size:11px;font-weight:600;color:#fff}')
  ...
  p.push('.bar-info{display:flex;flex-wrap:wrap;gap:6px;padding:10px 16px;background:#fff;border-bottom:1px solid #E2E8F0;font-size:12px}')
  p.push('.bar-info .b{background:#F8FAFC;border:1px solid #E2E8F0;border-radius:4px;padding:4px 10px}')
  p.push('.bar-info .l{color:#64748B;font-size:9.5px;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px}')
  ...
  p.push('td{padding:4px 10px;font-size:12px;border:1px solid #E2E8F0;width:25%}')
  p.push('td.l{font-weight:600;color:#475569;background:#F8FAFC;border-left:2px solid #E2E8F0}')
  p.push('td.v{color:#1E293B}')
  p.push('.vacio{color:#64748B;font-style:italic}')
  p.push('.s-n{color:#64748B} .s-v{color:#B91C1C;font-weight:600} .s-d{color:#15803D;font-weight:600}')
  ...
  p.push('.leyV{font-size:9.5px;color:#64748B;line-height:1.8;padding:0 2px}')
  ...
  p.push('.csvbox{margin:8px 16px 0;background:#fff;border:1px solid #E2E8F0;border-radius:4px;overflow:hidden}')
  ...
  p.push('.csvit{display:flex;justify-content:space-between;align-items:center;font-size:11px;padding:3px 0;border-bottom:1px solid #F1F5F9}')
  ...
  p.push('.ftr{font-size:10px;color:#64748B;text-align:center;padding:12px 16px;border-top:1px solid #E2E8F0;margin-top:8px;background:#fff}')
  ...
  p.push('<div class="b"><div class="l">Edad</div>' + _esc(edadTxt) + '</div>')
  ...
  p.push('<div style="font-size:9px;color:#64748B;margin-top:2px">Sistema PADDS V5.6 · Creado por Patricio Varela C. · Interno TENS · Contacto: patriciovarelacontreras@gmail.com</div>')
  p.push('</div>')
...
function _buildFichaHtml(headers, data, lc, autor) {
  ...
  var h = '<html><head><meta charset="UTF-8"><style>' +
    ...
    'body{font-family:Arial,Helvetica,sans-serif;font-size:9.5pt;color:#1E293B;line-height:1.45;margin:0;padding:0}' +
    ...
    '.info{background:#F8FAFC;padding:9px 20px;border-top:1px solid #E2E8F0;border-bottom:1px solid #E2E8F0;margin-bottom:12px}' +
    '.info sp{display:inline-block;margin-right:26px;font-size:9pt;color:#475569}' +
    '.info .lb{font-size:6.8pt;color:#64748B;text-transform:uppercase;letter-spacing:0.6px}' +
    ...
    'td.l{font-weight:bold;color:#475569;width:22%;background:#F8FAFC;border-left:2px solid #E2E8F0}' +
    'td.v{color:#1E293B;width:28%}' +
    '.vacio{color:#64748B;font-size:7.5pt;font-style:italic}' +
    '.s-v{color:#B91C1C;font-weight:bold} .s-d{color:#15803D;font-weight:bold}' +
    '.s-p{color:#C2410C;font-weight:bold} .s-x{color:#7E22CE;font-weight:bold} .s-n{color:#64748B}' +
    '.v-d,.v-pv,.v-vd,.v-pd,.v-na{display:inline-block;padding:0 5px;border-radius:2px;font-weight:700}' +
    '.v-d{background:#DCFCE7;color:#15803D}.v-pv{background:#FFEDD5;color:#C2410C}.v-vd{background:#FEE2E2;color:#B91C1C}.v-pd{background:#FEF3C7;color:#B45309}.v-na{background:#F1F5F9;color:#64748B}' +
    '.ftr{font-size:7pt;color:#64748B;text-align:center;padding-top:10px;border-top:1px solid #E2E8F0;margin-top:8px}' +
    '.wm{position:fixed;bottom:2mm;left:0;right:0;text-align:center;font-size:6.5pt;color:#CBD5E1}' +
    '</style></head><body>'
  ...
  h += _buildSeccionesHtml(headers, data, lc, vig, {
    ...
    lBgFor: function(si) { return si % 2 === 0 ? '#ffffff' : '#F8FAFC' },
    skip: function(c) { return c === COL.PRIORIDAD } })

  h += '<div class="ftr">Ficha generada el ' + _fmtFechaActual() + ' — Sistema PADDS V5.6 · Creado por Patricio Varela C. · Interno TENS · Contacto: patriciovarelacontreras@gmail.com</div>'
...
function ponerTooltipsPacientes() {
  ...
  var hdr = sh.getRange(3, 1, 1, lc)
  var notasActuales = hdr.getNotes()[0]
  var notas = []
  for (var c = 1; c <= lc; c++) {
    var col = _COLUMNAS[c]
    if (!col) { notas.push(notasActuales[c - 1] || ''); continue }
    var sec = ''
...
function limpiarEspaciosPacientes(confirmado) {
  ...
  if (total > 0) {
    sh.getRange(4, 1, lr - 3, lc).setValues(data)
    _invalidaIndicePacientes()
  }
...
function depurarEntradasDuplicadas(skipConfirm) {
  ...
  if (totalCel > 0) _invalidaIndicePacientes()
