// ════════════════════════════════════════════════════════
// ÍNDICE ▏07_Agenda.gs │ generación/limpieza de la agenda semanal de profesionales
...
function eliminarSemana() {
  ...
  ss.toast('Semana eliminada', 'PADDS', 2)
}
...
function limpiarSemanasPasadas() {
  ...
  var maxCol = GI[2] + 1
  var lr = sh.getLastRow()
  if (lr < 1) { ss.toast('Agenda vacía, no hay semanas que limpiar', 'Agenda', 2); return }
  var d = sh.getRange(1, 1, lr, maxCol).getValues()
  var hoy = new Date()
  ...
  if (aLimpiar.length === 0) {
    ss.toast('No hay semanas anteriores a hoy para limpiar', 'Agenda', 3)
    return
  }
  ...
  ss.toast('Limpiando ' + aLimpiar.length + ' semanas…', 'PADDS', 1)
  for (var _ai = 0; _ai < aLimpiar.length; _ai++) {
    limpiarBloque(sh, aLimpiar[_ai].fila, aLimpiar[_ai].col)
  }
  ss.toast(aLimpiar.length + ' semanas anteriores limpiadas', 'PADDS', 4)
}
...
function navegarA(s) {
  ...
  for (var r = 0; r < d.length; r++) {
    for (var ci = 0; ci < 3; ci++) {
      ...
      if (c < d[r].length && d[r][c] && _fmtCeldaParaComparar(d[r][c]).indexOf(s) === 0) {
        ...
        ss.toast('Encontrado: ' + s, 'Agenda', 2); return
      }
    }
  }
  for (var r = 0; r < d.length; r++) {
  ...
  SpreadsheetApp.getUi().alert('No encontrado', s + ' no encontrado en la agenda.', ui.ButtonSet.OK)
}
...
function buscarPaciente() {
  ...
  if (resultados.length === 0) { ui.alert('Sin resultados', 'No se encontró "' + q + '" en la agenda.', ui.ButtonSet.OK); return }
  if (resultados.length === 1) {
    sh.setActiveRange(sh.getRange(resultados[0].row, resultados[0].col))
    sh.getRange(resultados[0].row, resultados[0].col - 2, 1, 6)
      .setBorder(true, true, true, true, true, true)
    ss.toast('Paciente encontrado: ' + resultados[0].nombre, 'Agenda', 4); return
  }
  ...
  if (isNaN(idx) || idx < 0 || idx >= resultados.length) { ui.alert('Número inválido', 'Ingresa un número entre 1 y ' + resultados.length + '.', ui.ButtonSet.OK); return }
  ...
  sh.getRange(resultados[idx].row, resultados[idx].col - 2, 1, 6)
    .setBorder(true, true, true, true, true, true)
  ss.toast('Paciente encontrado: ' + resultados[idx].nombre, 'Agenda', 4)
}
